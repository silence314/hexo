---
title: JVM的内存结构相关总结
date: 2020-09-07 15:25:11
tags: 
 - Java 
 - JVM
 - JVM内存结构
description: JVM的内存结构相关总结
cover: /blogImg/JVM内存结构.jpg
categories: Java基础
...
首先，放张jvm架构图，看到感觉特别好，承上启下

![JVM架构图](/blogImg/JVM内存结构.jpg)
...
网上找了张图，可以看出如何通过参数来控制各区域的内存大小

![JVM内存](/blogImg/JVM内存.png)
-Xms设置堆的最小空间大小。
-Xmx设置堆的最大空间大小。
...
根据Java虚拟机规范的规定，当方法区无法满足内存分配需求时，将抛出OutOfMemoryError异常。**但是**：JDK8之后就没有永久代这一说法变成叫做**元空间（meta space）**，而且将老年代与元空间剥离。元空间放置于本地的内存中，因此元空间的最大空间就是系统的内存空间了，从而不会再出现像永久代的内存溢出错误了，也不会出现泄漏的数据移到交换区这样的事情。用户可以为元空间设置一个可用空间最大值，不设置默认根据类的元数据大小动态增加元空间的容量。对于一个 64 位的服务器端 JVM 来说，其默认的`–XX:MetaspaceSize` 值为 21MB。也就是说默认的元空间大小是21MB。
